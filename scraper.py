...
TOP_N_CONFIGS = 250 # <<<<<<<<<<<<<<<<<<<< تغییر در اینجا اعمال شد
...
def get_content_from_url(url: str): # ... (no change)
def decode_content(content: str): # ... (no change)
def discover_github_sources(): # ... (no change)
def parse_config(config_url: str): # ... (no change)
def tcp_ping(host: str, port: int, timeout: int = 2): # ... (no change)
def test_config_latency(config: str): # ... (no change)
def upload_to_gitlab(content: str): # ... (no change)
# Re-implementing functions here for completeness
...
def main():
    ...
    # Here you could add logic to query Supabase and combine scores
    # For now, we sort by our own ping test
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f: f.write(final_content)
