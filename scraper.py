...
def get_github_file_last_commit(url: str) -> datetime | None:
    ...
    if not match or not GITHUB_PAT:
        return None
...
# === FULL PROTOCOL SUPPORT (IMPROVED) ===
def parse_config(config_url: str) -> dict | None:
    try:
        if not any(config_url.startswith(prefix) for prefix in VALID_PREFIXES):
            return None
        ...
        if protocol == 'vmess':
            try:
                # اصلاح دیکد کردن: کل رشته بعد از vmess://
                raw_base64 = config_url[len('vmess://'):]
                decoded_part = safe_b64_decode(raw_base64).decode('utf-8')
            ...
            try:
                raw_part = config_url[len('ss://'):]
                if '#' in raw_part:
                    raw_part, remark_encoded = raw_part.split('#', 1)
                    remark = unquote(remark_encoded)
                
                # اصلاح دیکد کردن: مدیریت لینک‌های بدون Base64
                if '@' in raw_part:
                    userinfo, _, server_part = raw_part.rpartition('@')
                    decoded_userinfo = userinfo
                else:
                    decoded_part = safe_b64_decode(raw_part).decode('utf-8')
                    decoded_userinfo, _, server_part = decoded_part.rpartition('@')
        ...
        # برای پروتکل‌هایی که با urlparse به درستی کار می‌کنند
...
# === ADVANCED CLASH CONFIG GENERATION (IMPROVED) ===
        ...
        try:
            ...
            if parsed['protocol'] == 'vless':
                proxy.update({
                    ...
                    # اصلاح: پیش فرض network به tcp تغییر یافت
                    'network': params.get('type', 'tcp')
                })
                # اصلاح: اضافه کردن flow
                if 'flow' in params:
                ...
                if proxy['network'] == 'ws':
                    proxy['ws-opts'] = {
                        ...
                        'headers': {'Host': params.get('host', proxy.get('servername', parsed['server']))}
                    }
                ...
                if proxy['tls']:
                    proxy['servername'] = p.get('sni', parsed['server'])
                if proxy['network'] == 'ws':
                    proxy['ws-opts'] = {
                        'path': p.get('path', '/'),
                        'headers': {'Host': p.get('host', proxy.get('servername', parsed['server']))}
                    }
                ...
                proxy.update({
                    ...
                    'alpn': [p for p in params.get('alpn', 'h3').split(',') if p]
                })
                ...
                proxy.update({
                    ...
                    'disable_sni': params.get('disable_sni', 'false').lower() == 'true',
                    'reduce_rtt': params.get('reduce_rtt', 'false').lower() == 'true'
                })
            ...
            if 'shadowtls' in params:
                ...
                proxy['plugin-opts'] = {
                    'host': params.get('sni', parsed['server']),
                    'password': params.get('shadowtls'),
                    'version': '3'
                }
...
def get_operator_priority(config_name: str) -> int:
    ...
    if any(k in name_lower for k in ['mci', 'hamrah', 'همراه']):
        return 1
    if any(k in name_lower for k in ['irancell', 'ایرانسل']):
        return 2
    if any(k in name_lower for k in ['rightel', 'رایتل']):
        return 3
    return 99
...
def main():
    ...
    with open(OUTPUT_FILE_CLASH, 'w', encoding='utf-8') as f:
        f.write(generate_clash_config(top_configs_str))
