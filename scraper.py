# -*- coding: utf-8 -*-
"""
V2V Scraper v10.0 - DEBUG Edition
This version includes extensive logging to diagnose runtime issues.
"""
...
HEADERS = {'User-Agent': f'V2V-Scraper/v10.0-Debug','Cache-Control': 'no-cache', 'Pragma': 'no-cache', 'Expires': '0'}
...
if GITHUB_PAT:
    print(f"DEBUG: GH_PAT found. Length: {len(GITHUB_PAT)}")
    HEADERS['Authorization'] = f'token {GITHUB_PAT}'
else:
    print("CRITICAL_DEBUG: GH_PAT secret is NOT set in the environment!")
...
def fetch_and_parse_url(url: str) -> set:
    try:
        ...
        # print(f"DEBUG: Failed to fetch URL {url}. Error: {e}")
        return set()

def get_static_sources() -> list:
    print("DEBUG: Loading static sources...")
    try:
        with open(SOURCES_FILE, 'r', encoding='utf-8') as f:
            sources = json.load(f).get("static", [])
            print(f"DEBUG: Found {len(sources)} static sources.")
            return sources
    except Exception as e:
        print(f"CRITICAL_DEBUG: Could not read static sources file '{SOURCES_FILE}'. Error: {e}")
        return []

def discover_dynamic_sources() -> list:
    print("DEBUG: Attempting to discover dynamic sources from GitHub...")
    if not GITHUB_PAT:
        print("CRITICAL_DEBUG: Skipping dynamic source discovery because GH_PAT is missing.")
        return []
    ...
    try:
        ...
        user = g.get_user()
        print(f"DEBUG: Successfully authenticated to GitHub as user '{user.login}'.")
        print(f"DEBUG: Rate limit remaining: {g.get_rate_limit().core.remaining}")

        freshness_threshold = datetime.now(timezone.utc) - timedelta(hours=GITHUB_FRESHNESS_HOURS)
        for query in GITHUB_SEARCH_QUERIES:
            print(f"DEBUG: Searching GitHub for query: '{query}'")
            try:
                ...
                print(f"WARNING_DEBUG: GitHub API error during search for query '{query}'. This might be a rate limit issue. Error: {e}")
                continue
        print(f"DEBUG: Found {len(dynamic_sources)} dynamic sources from GitHub.")
        return list(dynamic_sources)

    except Exception as e:
        print(f"CRITICAL_DEBUG: A fatal error occurred during GitHub dynamic source discovery. Error: {e}")
        return []
...
def test_config_advanced(config_str: str) -> dict:
    ...
    try:
        ...
        if parsed_url.scheme == 'vmess':
            ...
            host = vmess_data.get('add'); port = int(vmess_data.get('port', 443)); is_tls = vmess_data.get('tls') == 'tls'; sni = vmess_data.get('sni', host)
        else:
            host = parsed_url.hostname; port = parsed_url.port; params = dict(parse_qsl(parsed_url.query)); is_tls = params.get('security') == 'tls' or parsed_url.scheme == 'trojan'; sni = params.get('sni', host)
...
def generate_clash_subscription(configs: list) -> str | None:
    # ... (No major changes needed, just adding a debug print) ...
    print("DEBUG: Generating Clash subscription...")
    ...
    if not proxies:
        print("DEBUG: No valid proxies found for Clash subscription.")
        return None
...
def main():
    print("--- V2V Scraper v10.0 (DEBUG MODE) ---")
    start_time = time.time()
    
    all_sources = list(set(get_static_sources() + discover_dynamic_sources()))
    print(f"DEBUG: Total unique sources to fetch: {len(all_sources)}")
    if not all_sources:
        print("CRITICAL_DEBUG: No sources found. Exiting.")
        return
    ...
    print(f"DEBUG: Total unique raw configs found after fetching: {len(raw_configs)}")
    if not raw_configs:
        print("CRITICAL_DEBUG: No raw configs could be parsed from any source. Exiting.")
        return
    ...
    print(f"DEBUG: Categorized configs: {len(xray_compatible_set)} Xray-compatible | {len(singbox_only_set)} Sing-box only")
    
    all_unique_configs = list(xray_compatible_set.union(singbox_only_set))
    configs_to_test = all_unique_configs[:MAX_CONFIGS_TO_TEST]
    
    print(f"DEBUG: Starting to test {len(configs_to_test)} configs...")
    ...
    print(f"DEBUG: Found {len(fast_configs_results)} fast configs after testing.")
    if not fast_configs_results:
        print("CRITICAL_DEBUG: No fast configs found. No output files will be generated.")
        return
    
    # ... (Balancing and processing logic - no changes needed) ...
    ...
    print("DEBUG: Preparing to write output files...")
    ...
    try:
        ...
        print(f"CRITICAL_DEBUG: Failed to write '{OUTPUT_JSON_FILE}'. Error: {e}")
    ...
    if clash_content:
        try:
            ...
            print(f"CRITICAL_DEBUG: Failed to write '{OUTPUT_CLASH_FILE}'. Error: {e}")
    
    try:
        with open(CACHE_VERSION_FILE, 'w') as f: f.write(str(int(time.time())))
        print(f"SUCCESS: '{CACHE_VERSION_FILE}' created successfully.")
    except Exception as e:
        print(f"CRITICAL_DEBUG: Failed to write '{CACHE_VERSION_FILE}'. Error: {e}")
